...
static int16_t gatt_temp_centi;
...
static ssize_t temp_read(struct bt_conn *c, const struct bt_gatt_attr *a,
    ...
    return bt_gatt_attr_read(c, a, buf, len, off, a->user_data, sizeof(gatt_temp_centi));
}
...
/* 2B company ID (Nordic 0x0059) + 2B temp in 0.01°C (LE) */
...
static struct bt_data ad[] = {
    BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
    BT_DATA_BYTES(BT_DATA_UUID16_ALL,
                  BT_UUID_16_ENCODE(BT_UUID_ESS_VAL),   /* <- add ESS */
                  BT_UUID_16_ENCODE(BT_UUID_BAS_VAL),
                  BT_UUID_16_ENCODE(BT_UUID_DIS_VAL)),
...
while (1) {
    ...
    /* ... (hrs_notify / bas_notify if you keep them) ... */

    /* --- DS18B20 read + GATT/ADV update --- */
    struct sensor_value t;   /* <-- add this line */
